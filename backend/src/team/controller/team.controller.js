...
const registerSchema = Joi.object({
  ...
  type: Joi.string().valid("BINUSIAN", "NON_BINUSIAN").required().messages({
...
const registerTeam = async (req, res, next) => {
  try {
    ...
    // Return readable validation errors to the frontend if Joi failed
    if (err && err.isJoi) {
    ...
    // Delegate other errors to centralized error handler
    next(err);
  }
};
...
const loginTeam = async (req, res, next) => {
  try {
    ...
    if (!team) {
      return res.status(401).json({ message: "Nama team atau password salah" });
    }
